...
$base_url = ($current_url === $current_url_check . '/videos/news-commentary/') ? '' : '/cancervax-inhouse';
...
$filteredNewsCommentaryVideos = array_filter($videos, function ($item) use ($videoTitleFromURL) {
    return $item['category'] === 'news-commentary' && $item['scope'] === 'public' && strtolower($item['title']) === $videoTitleFromURL;
});
...
if ($showAllVideo == 1) {
    ...
    if (count($filteredNewsCommentaryVideos) > 0) {
        // There are matching videos in $filteredNewsCommentaryVideos
